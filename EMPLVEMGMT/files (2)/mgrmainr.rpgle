     ...
     D MGRAPPLYR       PR                  EXTPGM('MGRAPPLYR')
     D  xEMPID                        4S 0
     D  xMODE                         1A
      ...
      //**** Main Dashboard Loop ***************************
           ...
           // Check Menu selection
               select;
                 ...
                 when $SELECT = 4;          // Apply Comp-Off (as employee)
                       W_ACTION = 'C';
                       MGRAPPLYR(W_MGRID : W_ACTION);
                       $SELECT = 0;

                 when $SELECT = 5;          // Apply Leaves (as employee)
                       W_ACTION = 'L';
                       MGRAPPLYR(W_MGRID : W_ACTION);
                       $SELECT = 0;

                 when $SELECT = 6;          // Leave Status
                       W_ACTION = 'S';
                       MGRAPPLYR(W_MGRID : W_ACTION);
                       $SELECT = 0;

                 when $SELECT = 7;          // Comp-Off Status
                       W_ACTION = 'X';
                       MGRAPPLYR(W_MGRID : W_ACTION);
                       $SELECT = 0;
